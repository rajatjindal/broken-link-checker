 ...
 */
const createRequest = (url, auth, method, options, retryCounter=0) => new Promise((resolve, reject) =>
{
	const redirects = [];
	...
	.on(RESPONSE_EVENT, stream =>
	{
		...
		if (retryCounter === 0 && method===HEAD && options.retryHeadFail && options.retryHeadCodes.includes(response.status))
		{
			// Retry potentially broken server with GET
			resolve( createRequest(url, auth, GET, options, retryCounter++) );
		}
		else if (options.retry && retryCounter < options.maxRetries && method === GET && options.retryCodes.includes(response.status))
			resolve( createRequest(url, auth, method, options, retryCounter++) );
		}
